...
def main():
    ...
    for cellk, (ds, at, tk) in CELLS.items():
        ...
        emb = np.load(ARTIFACTS / "fare_cells" / f"{FARE_CELLKEY[cellk]}_fare_embeddings.npz")
        for tier in (1, 2):
                ...
                for surf, ks in (("rep", ("mr", "ar", "sr")), ("out", ("mo", "ao", "so"))):
                    ...
                    print("  %-12s T%d %-4s macro=%.4f all=%.4f sup=%.4f  %s"
                          % (method, tier, surf, r["macro"], r["all_pairs"],
                             r["supported"],
                             "passes" if r["passes_supported"] else "FAILS"), flush=True)
                (RESULTS / "baseline_dual_score.json").write_text(
    ...
    (RESULTS / "baseline_dual_score.json").write_text(json.dumps(out, indent=1, default=float))
    print(f"\n[save] {RESULTS / 'baseline_dual_score.json'} ({out['minutes']} min)")
