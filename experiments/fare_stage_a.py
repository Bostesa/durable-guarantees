"""FARE gauntlet stage A — runs in the FARE venv ($FARE_ROOT/.venv,
...
Usage: cd "$FARE_ROOT/code" && ../.venv/bin/python \
  /path/to/durable-guarantees/experiments/fare_stage_a.py [cell ...]
(must run from fare/code so `from src.tree...` imports resolve).

$FARE_ROOT points at the official FARE checkout; it defaults to ../fare
...
# Official FARE checkout; defaults to ../fare beside this repository.
FARE_ROOT = Path(os.environ.get("FARE_ROOT", REPO.parent / "fare")).resolve()
if not FARE_ROOT.exists():
    raise FileNotFoundError(
        f"FARE checkout not found at {FARE_ROOT}. Set $FARE_ROOT to the "
        "official FARE clone.")

sys.path.insert(0, str(FARE_ROOT / "code"))
