...
def main():
    ...
    for f in sorted((ARTIFACTS / "fleet_shards").glob("*.json")):
    ...
    (RESULTS / "fleet_dual.json").write_text(json.dumps(out, indent=1, default=float))
    print(f"\n[save] {RESULTS / 'fleet_dual.json'}  ({len(rows)} rows)")
