...
Runs in the FNF venv ($FNF_ROOT/.venv, py3.11 + numpy 1.23.5 +
...
# Official eth-sri/fnf checkout; defaults to ../fnf beside this repository.
FNF_ROOT = Path(os.environ.get("FNF_ROOT", REPO.parent / "fnf")).resolve()
if not FNF_ROOT.exists():
    raise FileNotFoundError(
        f"FNF checkout not found at {FNF_ROOT}. Set $FNF_ROOT to the official "
        "eth-sri/fnf clone.")
OUT_DIR = ARTIFACTS / "fnf_cells"
...
def main():
    ...
    npz = np.load(ARTIFACTS / "fare_cells" / f"{a.cell}.npz")
