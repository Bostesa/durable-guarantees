...
Score arrays and shards under $DG_ARTIFACTS (default analysis/).
...
Outputs: analysis/tpr_scores/<point>.npz (compressed scores+labels),
results/tpr_capture.json (AUCs, TPR@FPR, reproduction gate, config).
"""
...
SCORE_DIR = ARTIFACTS / "tpr_scores"
...
def main():
    ...
    (RESULTS / "tpr_capture.json").write_text(json.dumps(out, indent=1))
    print(f"\n[save] {RESULTS / 'tpr_capture.json'} ({out['minutes']} min)", flush=True)
