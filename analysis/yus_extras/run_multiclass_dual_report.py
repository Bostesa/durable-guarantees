...
def main():
    ...
    print(hdr); print("-" * len(hdr))
    for f in sorted((HERE / "tpr59_scores").glob("*.npz")):
        stem = f.stem
    ...
    (HERE / "multiclass_dual_report.json").write_text(
        json.dumps(out, indent=1, default=float))
    print(f"\n[save] {HERE / 'multiclass_dual_report.json'}")
