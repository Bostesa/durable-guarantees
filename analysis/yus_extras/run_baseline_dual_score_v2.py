...
def main():
    t0 = time.time()
    device = get_device()
    rows = json.loads((HERE / "_baseline_partial.json").read_text())
    for r in rows:
    ...
    for cellk, method in ORDER:
        ...
        if method == "FARE":
            emb = np.load(HERE / "fare_cells" / f"{cellk}_fare_embeddings.npz")
        ...
        (HERE / "baseline_dual_score.json").write_text(json.dumps(dict(
    ...
    print(f"[save] {HERE / 'baseline_dual_score.json'} "
