...
def main():
    ...
    # ---------------- D4 ---------------------------------------------------
    dp = json.loads((HERE / "dp_fullrank.json").read_text())
    K = dp["K_gaussian"]
    ...
    (HERE / "absutility_dpeps.json").write_text(json.dumps(out, indent=1, default=float))
    print(f"\n[save] {HERE / 'absutility_dpeps.json'}")
