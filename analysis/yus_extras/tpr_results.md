...
2026-07-23. Script: `analysis/yus_extras/run_tpr.py` (new file; no existing script,
figure, or result file modified). Raw per-example scores:
`analysis/yus_extras/tpr_scores/*.npz` (14 files). All aggregates:
`analysis/yus_extras/tpr_capture.json`. Wall time: **41.7 min** on MPS.
...
- TPR convention: max TPR at FPR ≤ target (step function, no interpolation).
  ...
  that flag).
- Source for every number below: `analysis/yus_extras/tpr_capture.json`
