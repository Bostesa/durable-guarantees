...
Outputs: analysis/yus_extras/tpr_ext_scores/<group>.npz,
...
def save_section(section, group_arrays, records, gates, minutes):
    ...
    out = {"section": section, "records": records, "gates": gates,
           "minutes": minutes}
    (HERE / f"tpr_ext_partial_{section}.json").write_text(json.dumps(out))
...
def load_ckpt(section):
    p = HERE / f"tpr_ext_ckpt_{section}.json"
    if p.exists():
...
def merge():
    header = json.loads((HERE / "tpr_extension.json").read_text())
    records, gates = [], []
    sections_done = []
    for sec in ("a", "b1", "b2", "b3"):
        p = HERE / f"tpr_ext_partial_{sec}.json"
        if p.exists():
            d = json.loads(p.read_text())
    ...
    (HERE / "tpr_extension.json").write_text(json.dumps(header, indent=1))
